...
function Tasks({ allTasks }: { allTasks: TaskType[] }) {
  ...
  const completedCount =
    tasks?.filter((t) => getStatus(t) === "completed").length || 0;
  ...
  const saveChanges = async () => {
    try {
      ...
      const res = await updateMultipleStatuses({
        toComplete: toComplete as string[],
        toPending: toPending as string[],
      });
        ...
        {loading.fetchingData ? (
              ...
              return (
                <div className="task" key={task.id}>
                  ...
                  <button
                    ...
                    style={{
                      cursor: loading.deletingData[task.id as string]
                        ? "progress"
                        : "pointer",
                      padding: "0.5rem",
                    }}
          ...
          <div className="notasks">
            ...
            {categoryValue !== "All categories" ? (
              ...
              <p>
                No tasks with the category of{" "}
                <span className="filters">{categoryValue}</span> found.
              </p>
            ) : currentStatusFilter() ? (
              // CHECK 2: Only a path-based status filter is active and returned no tasks
              <p>
                No <span className="filters">{currentStatusFilter()}</span> task
                to display.
              </p>
            ) : (
